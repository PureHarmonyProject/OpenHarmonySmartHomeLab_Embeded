...
{
    hi_gpio_init();                                            // GPIO初始化
    hi_io_set_pull(ADC2_PIN, HI_IO_PULL_UP);                   // 设置GPIO上拉
}
...
{
    return hi_adc_convert_to_voltage(get_adc2_value());
}
...
{
    return hi_adc_convert_to_voltage(get_adc6_value());
}

void adc3_init(void)
{
    hi_gpio_init();                                            // GPIO初始化
    hi_io_set_pull(ADC3_PIN, HI_IO_PULL_UP);                   // 设置GPIO上拉
}
...
{
    uint16_t data;
    hi_adc_read(HI_ADC_CHANNEL_3,&data,HI_ADC_EQU_MODEL_8,HI_ADC_CUR_BAIS_DEFAULT,0xff);
    return data;
}
...
{
    return hi_adc_convert_to_voltage(get_adc3_value());
}