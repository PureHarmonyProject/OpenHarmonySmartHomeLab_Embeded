...
{   
    uint8_t retry=0;
    dht11_io_in(); 
    while (GPIO_GetInputValue(DHT11_PIN,&DHT11_DQ_IN)&&retry<100)//DHT11之后会拉低40~50us
    {
        retry++;
        hi_udelay(1);
    };   
...
uint8_t dht11_read_bit(void)             
{
    uint8_t retry=0;
    dht11_io_in();
    while(GPIO_GetInputValue(DHT11_PIN,&DHT11_DQ_IN)&&retry<100)//dht11拉高总线80us后开始传输数据，等待变为低电平 12-14us 开始
    {
        retry++;
        hi_udelay(1);
    }
    retry=0;
    while((!GPIO_GetInputValue(DHT11_PIN,&DHT11_DQ_IN))&&retry<100)//50us低电平帧头，等待变高电平    26-28us表示0,116-118us表示1
    {
        retry++;
        hi_udelay(1);
    }
    hi_udelay(40);//等待40us
    if(GPIO_GetInputValue(DHT11_PIN,&DHT11_DQ_IN))return 1;//四十us后检测，高电平说明处在70us的1中，低电平说明处于50us的帧头中
    else return 0;         
}
